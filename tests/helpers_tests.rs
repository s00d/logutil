...
async fn test_process_line() {
    let log_data = Arc::new(Mutex::new(LogData::with_enabled_tabs(
        true, true, true, true, true, true,
    )));
    let regex_pattern = r#"^(\S+) - \S+ \[(.*?)\] \d+\.\d+ "(\S+)" "(\S+) (\S+?)(?:\?.*?)? "#;
    let date_format = "%d/%b/%Y:%H:%M:%S %z";
    let test_line =
        r#"127.0.0.1 - - [10/Oct/2023:13:55:36 +0000] 0.000 "GET" "GET /test HTTP/1.1" "#;

    process_line(test_line, regex_pattern, date_format, &log_data)
        .await
        .unwrap();
...
async fn test_process_line_invalid_format() {
    ...
    process_line(invalid_line, regex_pattern, date_format, &log_data)
        .await
        .unwrap();
...
127.0.0.2 - - [10/Oct/2023:13:55:37 +0000] 0.000 "GET" "GET /test2 HTTP/1.1" "#,
...

    {
        ...
        writeln!(
            file,
            r#"127.0.0.3 - - [10/Oct/2023:13:55:38 +0000] 0.000 "GET" "GET /test3 HTTP/1.1" "#
        )
        .unwrap();
    }
